...
class Misc(commands.Cog):
    ...
    @commands.command()
    async def emne(self, ctx, emnekode: str):

        data = requests.get(f"https://data.uio.no/studies/v1/course/{emnekode}")
        if data.status_code != 200:
            return await ctx.send(embed=embed_templates.error_fatal(ctx, text="Fant ikke emnekode"))

        emne = data.json()["info"]["name"]
        await ctx.send(emne)
    ...
    async def identifiser(self, ctx, bilde=None):
        ...
        async with ctx.channel.typing():
            ...
            url = (
                "https://api.clarifai.com/v2/models/aaa03c23b3724a16a56b629203edc62c"
    ...
    async def match(self, ctx, *, bruker: discord.Member = None):
        ...
        if bruker == ctx.author:
            embed = embed_templates.error_warning(
                ctx, text="Jeg vet du er ensom, men du kan " "ikke matche med deg selv"
            )
            return await ctx.send(embed=embed)
